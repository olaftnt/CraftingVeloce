...
public class CraftingVeloceMod {
    ...
    public CraftingVeloceMod(IEventBus modEventBus, ModContainer modContainer) {
        ...
        // The casing ITEM models: put the real icon of the represented block inside the
        // frame. Both models are already baked and in this map, so each row is two
        // lookups - no second bake, no renderers, nothing per frame.
        //
        // The 44 JSONs used to carry a small cube whose six faces all pointed at one
        // wall of the machine's texture, so a crafting table wore its front panel on its
        // lid. Those elements are gone; this puts the machine's own item model there
        // instead.
        modEventBus.addListener(net.neoforged.neoforge.client.event.ModelEvent.ModifyBakingResult.class, event -> {
            int composed = 0;
            int fellBack = 0;
            ...
            for (com.craftingveloce.block.VeloceCaseContents.Entry entry
                ...
                net.minecraft.world.item.Item machineItem = entry.machine().get().asItem();
                net.minecraft.world.item.Item contentItem = entry.content().get().asItem();
                if (machineItem == net.minecraft.world.item.Items.AIR
                        || contentItem == net.minecraft.world.item.Items.AIR) {
                    fellBack++;
                    missing.add(entry.machine().get().toString());
                    continue;
                }
                net.minecraft.client.resources.model.ModelResourceLocation caseKey =
                        net.minecraft.client.resources.model.ModelResourceLocation.inventory(
                                net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(machineItem));
                net.minecraft.client.resources.model.ModelResourceLocation contentKey =
                        net.minecraft.client.resources.model.ModelResourceLocation.inventory(
                                net.minecraft.core.registries.BuiltInRegistries.ITEM.getKey(contentItem));
                var caseModel = event.getModels().get(caseKey);
                var contentModel = event.getModels().get(contentKey);
                if (caseModel == null || contentModel == null) {
                    fellBack++;
                    missing.add((caseModel == null ? "case " : "content ")
                            + net.minecraft.core.registries.BuiltInRegistries.ITEM
                                    .getKey(contentItem));
                    continue;
                }
                event.getModels().put(caseKey, new com.craftingveloce.client.render.VeloceCaseItemModel(
                        caseModel, contentModel, entry.scale(), entry.pitch()));
                composed++;
            }
            // Logged, because a casing that fell back looks exactly like one that worked
            // before this change: it shows the plain frame with nothing inside it.
            LOGGER.info("[Veloce] casing icons: {} composed, {} fell back to the plain frame{}",
                    composed, fellBack,
